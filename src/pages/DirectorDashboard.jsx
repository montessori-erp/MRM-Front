...
export default function DirectorDashboard() {
  ...
  const updateBudgetMutation = useMutation({
    ...
    onSuccess: () => {
      messageApi.success('Budget updated successfully');
      // This invalidates all queries starting with ['dashboard'], 
      // refreshing both 'spending' and 'totalSpent'
      queryClient.invalidateQueries({ queryKey: ['dashboard'] }); 
      setIsModalOpen(false);
    },
    onError: (error) => {
      console.error("Update Error:", error);
      messageApi.error('Failed to update budget. Check console for details.');
    },
  });
  ...
  const handleSave = () => {
    if (!editingDept?._id) {
        messageApi.warning("Invalid Department ID");
        return;
    }
    updateBudgetMutation.mutate({ 
      id: editingDept._id, 
  ...
  return (
    <div style={{ padding: '20px' }}>
        ...
        <Table
          ...
          columns={[
            ...
            { 
                ...
                render: (v) => (
                    <Text type={v < 0 ? 'danger' : 'success'} strong>
                        {v < 0 ? `(Over) ₹${Math.abs(v).toLocaleString()}` : `₹${(v || 0).toLocaleString()}`}
                    </Text>
                )
            },
            {
      ...
      <Modal
        ...
        onOk={handleSave}
        onCancel={() => setIsModalOpen(false)}
        confirmLoading={updateBudgetMutation.isPending} // 'isPending' for React Query v5
        okText="Update & Save"
        cancelText="Cancel"
        destroyOnClose // Ensures state is fresh when opened
      >
        <div style={{ padding: '10px 0' }}>
          <Text type="secondary">Department:</Text>
          <Title level={4} style={{ marginTop: 0 }}>{editingDept?.name}</Title>
